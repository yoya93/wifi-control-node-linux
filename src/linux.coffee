...
(function() {
  ...
  parsePatterns = {
    ...
    nmcli_line: new RegExp(/^(\w+)\s+wifi\s+(\w+)\s+([^$]+)$/)
  };
  ...
  module.exports = {
    autoFindInterface: function() {
      ...
      this.WiFiLog("Host machine is Linuxxx");
      //findInterfaceCom = "nmcli -m multiline device status | grep wlan";
      
      // Quickfix for wifi devices not prefixed wlan
      //findInterfaceCom = "nmcli -m multiline device status | grep 'DEVICE:\\s\\+wl'";
      
      // Better fix
      findInterfaceCom = "nmcli d | grep 'wifi'";
    ...
    getIfaceState: function() {
      var KEY, VALUE, connectionData, connectionName, error, error1, error2, foundInterface, i, interfaceState, k, len, ln, parsedLine, powerData, ref, ssidData;
      interfaceState = {};
